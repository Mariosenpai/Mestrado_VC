...
from src.config.config import porcentagem_teste, arquivos_ignorados_CVMPT
...
class CV_mozilla_PT_Dataset(torch.utils.data.Dataset):
    ...
    def __init__(
            ...
            path: str,
            validacao=False,
            sample_rate_final=22050,
            duracao_min_segundos=3,
            return_mel_spec=False,
            is_test=False,
            ext: str = "mp3",
            path_duracoes_tsv_csv: str = r"C:\Users\USER\Documents\Mestrado\codigo\Mestrado_VC\dataset\cv-corpus-mozilla-pt\pt\clip_durations.tsv",
            file_mel_save_path: str = fr"C:\Users\USER\Documents\Mestrado\codigo\Mestrado_VC\dataset\cv-corpus-mozilla-pt\pt\mel"
        ...
        if not self.retorne_mel_spec:
            for idx in tqdm(range(len(self.index_map)), desc='Generando mel-spectrograma'):

                audio, segment, sr, segment_size, file_name = self.audio_info(idx, False)

                # Pegando apenas o nome do arquivo
                file_name = os.path.basename(file_name)
                file_name = file_name[:len(file_name) - 4]

                print(audio.shape, segment.shape, sr, segment_size)

                if len(segment) == segment_size:
                    spectrogram = self.gerar_mel_spectograma(audio, sr)
                    ...
                    img = Image.fromarray(spectrogram.numpy(), mode='RGB')
                    img.save(os.path.join(file_mel_save_path, file_name + '.png'))

                    if is_test: break



    def gerar_mel_spectograma(self, audio, sr):
        audio_cortado = self.corta_audio(audio, sr)
    ...
    def audio_info(self, idx, mostrar_path):
    ...
    def __getitem__(self, idx, mostrar_path=False):
        try:
            ...
            audio, segment, sr, segment_size, file_path = self.audio_info(idx, mostrar_path)
...
if __name__ == '__main__':
    data = CV_mozilla_PT_Dataset(
        path=r"/dataset/cv-corpus-mozilla-pt/pt/clips",
        is_test=True
    )
