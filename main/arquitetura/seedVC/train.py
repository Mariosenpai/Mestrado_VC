...
from bibliotecas_externas.seed_vc.modules.commons import recursive_munch, build_model, load_checkpoint
from bibliotecas_externas.seed_vc.optimizers import build_optimizer
from bibliotecas_externas.seed_vc.data.ft_dataset import build_ft_dataloader
from bibliotecas_externas.seed_vc.hf_utils import load_custom_model_from_hf
from main.arquitetura.seedVC.inferencia import voice_conversion_inference
...
class Trainer:
    def __init__(self,
        ...
        self.train_dataloader, self.val_dataloader, self.test_dataloader = build_ft_dataloader(
            data_dir,
        ...
        print("tamanho dataset :", self.train_dataloader.__len__())
        ...
        self.model_params["noise_gaussian"] = noise_gaussian
    ...
    def avaliable(self, dataloader):
        ...
        for batch in tqdm(dataloader):
            ...
            waves, mels, mel_noise, wave_lengths, mel_input_length = batch

            x, target_lengths, prompt_len, cond, y, alt_commitment_loss, ori_commitment_loss, ori_codebook_loss, alt_codebook_loss = self.pre_process_waves(
                waves, mels, wave_lengths, mel_input_length
            )

            # inferencia
            loss, output_model = self.model.cfm(x, target_lengths, prompt_len, cond, y, mel_noise)

            # pega aleatoriamente 10 amostras de test
            if random.randint(1, 2) == 1 and len(samples_test) >= 10:
                samples_test.append((x, output_model))

            from main.metricas import mcd, lsd, wer, psnr, lpips

            for i, output in enumerate(output_model):
                output_np = Tensor(output).detach().cpu().numpy()
                mel_np = Tensor(mels[i]).detach().cpu().numpy()

                min_size = min(mel_np.shape[1], output_np.shape[1])  # pegar o menor tamanho
                mel = mel_np[:, :min_size]  # Truncar mel
                mel_output = output_np[:, :min_size]  # Truncar mel_noise

                mcd_m += mcd(mel, mel_output)
                lsd_m += lsd(mel, mel_output)
                #wer_m += wer(mel_np, output_np, self.sr)
                psnr_m += psnr(mel, mel_output)
                lpinp_m += lpips(mel, mel_output)
        ...
        #print(f'WER: {wer_m}')
        print(f'PSNR: {psnr_m}')
        print(f"LPIPS: {lpinp_m}")
        # print(f"WER: {wer_m}")
        history["mcd"] = float(mcd_m)
        history["lsd"] = float(lsd_m)
        # history["wer_m"] = float(wer_m)
        history["psnr_m"] = float(psnr_m)
        history["lpinp_m"] = float(lpinp_m)
        history["loss"] = float(loss)

        history_epochs.append((self.epoch, history))

        return history_epochs, samples_test

    def build_sv_model(self, device, config):
        from bibliotecas_externas.seed_vc.modules.campplus.DTDNN import CAMPPlus
    ...
    def build_f0_fn(self, device, config):
        from bibliotecas_externas.seed_vc.modules.rmvpe import RMVPE
    ...
    def build_converter(self, device, config):
        from bibliotecas_externas.seed_vc.modules.openvoice.api import ToneColorConverter
        ckpt_converter, config_converter = load_custom_model_from_hf("myshell-ai/OpenVoiceV2",
    ...
    def build_vocoder(self, device, config):
        ...
        if vocoder_type == 'bigvgan':
            from bibliotecas_externas.seed_vc.modules.bigvgan import bigvgan
            ...
            from bibliotecas_externas.seed_vc.modules.hifigan.generator import HiFTGenerator
            from bibliotecas_externas.seed_vc.modules.hifigan.f0_predictor import ConvRNNF0Predictor
            hift_config = yaml.safe_load(open(
                r'C:\Users\USER\Documents\Mestrado\codigo\Mestrado_VC\bibliotecas_externas\seed_vc\configs\hifigan.yml',
                'r'))
    ...
    def pre_process_waves(self, waves, mels, wave_lengths, mel_input_length):
        ...
        if self.f0_constant:
            wave_f0_const = []
            for w in waves_22k:
                wave_f0_const.append(
                    torch.tensor(f0_constante(w.detach().cpu().numpy().astype(np.float64), self.sr)).float().unsqueeze(
                        0))
            wave_f0_const = torch.cat(wave_f0_const, dim=0)

            converted_waves_22k = wave_f0_const
        else:
        ...
        return x, target_lengths, prompt_len, cond, y, alt_commitment_loss, ori_commitment_loss, ori_codebook_loss, alt_codebook_loss
    ...
    def val_step(self):
        history_epochs, _ = self.avaliable(self.val_dataloader)
        with open(f'{self.log_dir}/history.txt', 'a') as f:
            f.write(f"{str(history_epochs)}\n")

    def test_step(self):
        history_epochs, sample_test = self.avaliable(self.test_dataloader)

        with open(f'{self.log_dir}/history.txt', 'a') as f:
            f.write(f"{str(history_epochs)}\n")

        import soundfile as sf
        for i, x, output_model in enumerate(zip(sample_test)):
            # Salvar o áudio resultante
            sf.write(f'{self.log_dir}/{i}_source.wav', x, self.sr)
            sf.write(f'{self.log_dir}/{i}_output.wav', output_model, self.sr)
...
if __name__ == '__main__':
    ...
    config_path = r'C:\Users\USER\Documents\Mestrado\codigo\Mestrado_VC\main\arquitetura\seedVC\output\runs\cv_mozila_pt_5_8_0.1_no_pre_train_xlsr_4\config_dit_mel_seed_uvit_xlsr_tiny.yml'
    pretrained_ckpt_path = models[1]
    ...
    if pretrained_ckpt_path is None:
        pre_train = "no_pre_train"
    else:
        pre_train = "pre_train"

    run_name = f'test_trein_timbre_f0_test_timbre_shifter_xlsr_5'
